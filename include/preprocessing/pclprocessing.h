...
namespace pre {
    ...
    public:
        ...
        bool initPCL(const pcl::PointCloud<PointT> &cloud, const std::string viewerName = "PCL Viewer"); // 初始化PCL(从点云中读取,并能修改视图名字)
        ...
        template <typename PointT>
        bool addPointCloud(const pcl::PointCloud<PointT> &cloud, const std::string &frameName); // 添加点云,并设置帧名字
        bool addCamera(const cv::Mat &R, const cv::Mat &t, int frameId, const std::string &imagePath = ""); // 添加相机(主要用于SFM运动结构恢复)
        void visualizeCameraInPointCloud(const CameraInfo &camera); // 在点云中可视化相机
        void setGlobalCloud(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr &cloud); // 设置全局点云
