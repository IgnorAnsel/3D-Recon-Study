...
#include <iostream>
void createCameraPath(pc::PCLProcessing& pclProcessor, int numCameras = 20);
int main() {
    // 创建PCLProcessing实例
    pc::PCLProcessing pclProcessor;
...
// 创建沿路径移动的相机序列
void createCameraPath(pc::PCLProcessing& pclProcessor, int numCameras) {
    // 初始外参矩阵 [R|t]
    cv::Mat extrinsic = cv::Mat::eye(4, 4, CV_64F);
