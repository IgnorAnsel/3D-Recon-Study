...
using namespace cv;
using namespace std;
Ptr<cv::SIFT> sift = cv::SIFT::create(1000, // 增加特征点数量
                                      3,
                                      0.03, // 调低对比度阈值
                                      15,   // 调整边缘阈值
                                      1.6);
...
void detectFeatures(const Mat &image, vector<KeyPoint> &keypoints,
                    Mat &descriptors) {
...
int main() {
  ...
  std::string CameraParamsPath_ =
      std::string(RESOURCE_DIR) + "/room/calibration_result.yml";
  ...
  Mat matchImg =
      drawFeatureMatches(img1, keypoints1, img2, keypoints2, goodMatches);
